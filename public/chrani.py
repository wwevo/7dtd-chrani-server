...
def find_key_by_value(d, mime_type):
        ...
        while not self.stopped.wait(0.5):
            ...
            if self.timeout_in_seconds != 0:
                ...
                if elapsed_time >= self.timeout_in_seconds:
                    ...
                    if statement, cause the event it might not be present
                    """
            ...
            if m:
                player = m.group(3)
                command = m.group(4)
                steamid = find_key_by_value(players, player)[0]
                if command == "make this my home":
                    home_x = players[steamid]['pos']['x']
                    home_y = players[steamid]['pos']['y']
                    home_z = players[steamid]['pos']['z']
                    player_locations[steamid] = {}
                    player_locations[steamid]['homePos'] = {}
                    player_locations[steamid]['homePos']['x'] = home_x
                    player_locations[steamid]['homePos']['y'] = home_y
                    player_locations[steamid]['homePos']['z'] = home_z
                    send_message(player + " has decided to settle down!")
                elif command == "take me home":
                    home_x = int(float(player_locations[steamid]['homePos']['x']))
                    home_y = int(float(player_locations[steamid]['homePos']['y']))
                    home_z = int(float(player_locations[steamid]['homePos']['z']))
                    ...
                    if p:
                        ...
                        if password == "letmein":
                            ...
                            spawn_x = int(float(player_locations[steamid]['spawnPos']['x']))
                            spawn_y = int(float(player_locations[steamid]['spawnPos']['y']))
                            spawn_z = int(float(player_locations[steamid]['spawnPos']['z']))
            ...
            if m:
                ...
                home_z = players[steamid]['pos']['z']
                player_locations[steamid] = {}
                player_locations[steamid]['spawnPos'] = {}
                player_locations[steamid]['spawnPos']['x'] = home_x
                player_locations[steamid]['spawnPos']['y'] = home_y
                player_locations[steamid]['spawnPos']['z'] = home_z
